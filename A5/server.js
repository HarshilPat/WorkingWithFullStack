...
con.connect((err) => {
    ...
    con.query("USE posts;", (err, result) => {
...
function post(topic,data) {
    let statement = `INSERT INTO data (Topic, Data, Timestamp) VALUES ('${topic}','${data}','${new Date()}')`
...
app.get('/getTopicAsc',(req,res) => {
    con.query(`SELECT * FROM data ORDER BY Topic`, function(err,result){
        if (err){
            console.error(err)
            return
        }
...
app.get('/getTopicDsc',(req,res) => {
    con.query(`SELECT * FROM data ORDER BY Topic DESC`, function(err,result){
        if (err){
            console.error(err)
            return
        }
...
app.get('/getTimeAsc',(req,res) => {
    con.query(`SELECT * FROM data ORDER BY Timestamp`, function(err,result){
        if (err){
            console.error(err)
            return
        }
...
app.get('/getTimeDsc',(req,res) => {
    con.query(`SELECT * FROM data ORDER BY Timestamp DESC`, function(err,result){
        if (err){
            console.error(err)
            return
        }
